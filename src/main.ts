...
import {AYXDeployResults, main} from './common'
...
async function run(): Promise<void> {
  try {
    ...
    const user_mail: string = core.getInput('ayx-user-mail')
    const folder_to_sync: string = core.getInput('folder-to-sync')
    ...
    const rslt: AYXDeployResults = await main(
      cClient,
      wClient,
      jClient,
      userClient,
    ...
    fs.writeFileSync(testReportFile, JSON.stringify(rslt, null, 4))
